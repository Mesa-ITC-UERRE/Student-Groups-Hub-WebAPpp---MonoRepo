...
public class EventService(AppDbContext db)
{
    ...
    public async Task<Event?> GetByIdAsync(Guid id)
        => await db.Events.Include(e => e.Group).FirstOrDefaultAsync(e => e.Id == id);
    ...
    public async Task<EventParticipation?> UpsertRsvpAsync(Guid eventId, Guid userId, string status)
    {
        ...
        {
            var count = await db.EventParticipations
    ...
    public async Task<int> GetRsvpCountAsync(Guid eventId)
        => await db.EventParticipations.CountAsync(p => p.EventId == eventId && p.Status == "going");
