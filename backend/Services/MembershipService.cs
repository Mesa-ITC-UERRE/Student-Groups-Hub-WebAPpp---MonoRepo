...
public class MembershipService(AppDbContext db)
{
    ...
    {
        // Enforce no-duplicate active request
        ...
        if (existing is not null)
        {
            ...
            // Allow re-apply after rejection/removal
    ...
    {
        var m = await db.Memberships.Include(m => m.User).FirstOrDefaultAsync(m => m.Id == membershipId);
